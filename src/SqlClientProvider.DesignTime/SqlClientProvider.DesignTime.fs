module SqlClientProviderImplementation
...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("SqlClientProvider.DesignTime", "SqlClientProvider.Runtime")], addDefaultProbingLocation=true)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("SqlClientProvider.DesignTime", "SqlClientProvider.Runtime")])

    let ns = "SqlClientProvider"
    let asm = Assembly.GetExecutingAssembly()
